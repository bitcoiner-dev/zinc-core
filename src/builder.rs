    ...
    pub fn derive_public_key_internal(
        ...
        match &self.kind {
            WalletKind::Seed { master_xprv } => {
                ...
                // SECURITY: Safely parse potentially untrusted derivation indices to prevent DoS panics from .unwrap() bounds errors
                let derivation_path = [
                    bdk_wallet::bitcoin::bip32::ChildNumber::from_hardened_idx(purpose).map_err(|e| format!("Invalid purpose index: {e}"))?,
                    bdk_wallet::bitcoin::bip32::ChildNumber::from_hardened_idx(coin_type).map_err(|e| format!("Invalid coin type index: {e}"))?,
            ...
            WalletKind::Hardware {
                ...
                let derived_xpub = xpub
                    .derive_pub(
                        &secp,
                        &[
                            // SECURITY: Safely handle derivation indices to prevent DoS panics
                            ChildNumber::from_normal_idx(0).map_err(|e| format!("Invalid derivation index: {e}"))?,
                            ChildNumber::from_normal_idx(index).map_err(|e| format!("Invalid derivation index: {e}"))?,
                        ],
                    )
