...
impl ZincWallet {
    ...
    pub fn sign_psbt(
        ...
        let mut known_inscriptions: HashMap<(bitcoin::Txid, u32), Vec<(String, u64)>> =
            HashMap::with_capacity(self.inscriptions.len()); // PERFORMANCE OPTIMIZATION (Bolt): Pre-allocate map capacity using known inscriptions length to avoid dynamic reallocations
