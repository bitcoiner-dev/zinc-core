    ...
    pub fn derive_public_key_internal(
        ...
        match &self.kind {
            WalletKind::Seed { master_xprv } => {
                ...
                // SECURITY: Avoid panic by returning an error if user-controlled derivation bounds exceed 2^31
                let derivation_path = [
            ...
            WalletKind::Hardware {
                ...
                let derived_xpub = xpub
                    .derive_pub(
                        &secp,
                        &[
                            ChildNumber::from_normal_idx(0).map_err(|e| format!("Invalid chain: {e}"))?,
                            // SECURITY: Avoid panic by returning an error if index exceeds 2^31
