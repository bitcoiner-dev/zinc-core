        ...
        match &self.kind {
            WalletKind::Seed { master_xprv } => {
                ...
                let chain = 0; // External
                                ...
                                // SECURITY: Avoid panic by returning an error if derivation indices are invalid (e.g., > 2^31)
                let derivation_path = [
                    bdk_wallet::bitcoin::bip32::ChildNumber::from_hardened_idx(purpose).map_err(|e| format!("Invalid derivation path: {}", e))?,
                    bdk_wallet::bitcoin::bip32::ChildNumber::from_hardened_idx(coin_type).map_err(|e| format!("Invalid derivation path: {}", e))?,
                    bdk_wallet::bitcoin::bip32::ChildNumber::from_hardened_idx(account).map_err(|e| format!("Invalid derivation path: {}", e))?,
                    bdk_wallet::bitcoin::bip32::ChildNumber::from_normal_idx(chain).map_err(|e| format!("Invalid derivation path: {}", e))?,
                    bdk_wallet::bitcoin::bip32::ChildNumber::from_normal_idx(index).map_err(|e| format!("Invalid derivation path: {}", e))?,
                ];
            ...
            WalletKind::Hardware {
                ...
                let derived_xpub = xpub
                    .derive_pub(
                        &secp,
                        // SECURITY: Avoid panic by returning an error if derivation indices are invalid
                        &[
                            ChildNumber::from_normal_idx(0).map_err(|e| format!("Invalid derivation path: {}", e))?,
                            ChildNumber::from_normal_idx(index).map_err(|e| format!("Invalid derivation path: {}", e))?,
                        ],
                    )
