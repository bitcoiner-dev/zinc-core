    ...
    pub fn sign_message(&self, address: &str, message: &str) -> Result<String, String> {
        ...
        let derivation_path = [
            bdk_wallet::bitcoin::bip32::ChildNumber::from_hardened_idx(purpose)
                .map_err(|e| format!("Invalid derivation index {purpose}: {e}"))?,
            bdk_wallet::bitcoin::bip32::ChildNumber::from_hardened_idx(coin_type)
                .map_err(|e| format!("Invalid derivation index {coin_type}: {e}"))?,
            bdk_wallet::bitcoin::bip32::ChildNumber::from_hardened_idx(account)
                .map_err(|e| format!("Invalid derivation index {account}: {e}"))?,
            bdk_wallet::bitcoin::bip32::ChildNumber::from_normal_idx(chain)
                .map_err(|e| format!("Invalid derivation index {chain}: {e}"))?,
            bdk_wallet::bitcoin::bip32::ChildNumber::from_normal_idx(index)
                .map_err(|e| format!("Invalid derivation index {index}: {e}"))?,
        ];
    ...
    fn sign_inscription_script_paths(
        ...
        let derivation_path = [
            bdk_wallet::bitcoin::bip32::ChildNumber::from_hardened_idx(86)
                .map_err(|e| format!("Invalid derivation index 86: {e}"))?,
            bdk_wallet::bitcoin::bip32::ChildNumber::from_hardened_idx(coin_type)
                .map_err(|e| format!("Invalid derivation index {coin_type}: {e}"))?,
            bdk_wallet::bitcoin::bip32::ChildNumber::from_hardened_idx(self.account_index)
                .map_err(|e| format!("Invalid derivation index {}: {e}", self.account_index))?,
            bdk_wallet::bitcoin::bip32::ChildNumber::from_normal_idx(0)
                .map_err(|e| format!("Invalid derivation index 0: {e}"))?, // External chain
            bdk_wallet::bitcoin::bip32::ChildNumber::from_normal_idx(0)
                .map_err(|e| format!("Invalid derivation index 0: {e}"))?, // First key
        ];
