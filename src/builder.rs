    ...
    pub fn derive_public_key_internal(
        ...
        match &self.kind {
            WalletKind::Seed { master_xprv } => {
                ...
                let derivation_path = [
                    // SECURITY: Avoid panic by returning an error if derivation indices exceed bounds (>= 2^31)
                    bdk_wallet::bitcoin::bip32::ChildNumber::from_hardened_idx(purpose).map_err(|e| format!("Invalid derivation path index: {}", e))?,
                    bdk_wallet::bitcoin::bip32::ChildNumber::from_hardened_idx(coin_type).map_err(|e| format!("Invalid derivation path index: {}", e))?,
                    bdk_wallet::bitcoin::bip32::ChildNumber::from_hardened_idx(account).map_err(|e| format!("Invalid derivation path index: {}", e))?,
                    bdk_wallet::bitcoin::bip32::ChildNumber::from_normal_idx(chain).map_err(|e| format!("Invalid derivation path index: {}", e))?,
                    bdk_wallet::bitcoin::bip32::ChildNumber::from_normal_idx(index).map_err(|e| format!("Invalid derivation path index: {}", e))?,
                ];
            ...
            WalletKind::Hardware {
                ...
                let derived_xpub = xpub
                    .derive_pub(
                        ...
                        &[
                            // SECURITY: Avoid panic by returning an error if derivation indices exceed bounds (>= 2^31)
                            ChildNumber::from_normal_idx(0).map_err(|e| format!("Invalid derivation path index: {}", e))?,
                            ChildNumber::from_normal_idx(index).map_err(|e| format!("Invalid derivation path index: {}", e))?,
                        ],
                    )
