    ...
    pub fn derive_public_key_internal(
        ...
        match &self.kind {
            WalletKind::Seed { master_xprv } => {
                ...
                // SECURITY: Use map_err on index derivation instead of unwrap to prevent DoS from maliciously large inputs
                let derivation_path = [
                    ...
                    bdk_wallet::bitcoin::bip32::ChildNumber::from_normal_idx(index).map_err(|e| format!("Invalid child index: {e}"))?,
                ];
            ...
            WalletKind::Hardware {
                ...
                let derived_xpub = xpub
                    .derive_pub(
                        &secp,
                        &[
                            // SECURITY: Safe parsing of index for hardware derived paths
                            ChildNumber::from_normal_idx(0).map_err(|e| format!("Invalid chain index: {e}"))?,
                            ChildNumber::from_normal_idx(index).map_err(|e| format!("Invalid child index: {e}"))?,
                        ],
                    )
