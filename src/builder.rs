    ...
    pub fn derive_public_key_internal(
        ...
        match &self.kind {
            WalletKind::Seed { master_xprv } => {
                ...
                // SECURITY: Avoid panic by returning an error if dynamic derivation indices exceed bounds
                let derivation_path = [
                    bdk_wallet::bitcoin::bip32::ChildNumber::from_hardened_idx(purpose).map_err(|_| "Invalid purpose".to_string())?,
                    bdk_wallet::bitcoin::bip32::ChildNumber::from_hardened_idx(coin_type).map_err(|_| "Invalid coin_type".to_string())?,
                    bdk_wallet::bitcoin::bip32::ChildNumber::from_hardened_idx(account).map_err(|_| "Invalid account".to_string())?,
                    bdk_wallet::bitcoin::bip32::ChildNumber::from_normal_idx(chain).map_err(|_| "Invalid chain".to_string())?,
                    bdk_wallet::bitcoin::bip32::ChildNumber::from_normal_idx(index).map_err(|_| "Invalid index".to_string())?,
                ];
                ...
                let derived_xpub = xpub
                    .derive_pub(
                        ...
                        &[
                            ...
                            // SECURITY: Avoid panic by returning an error if dynamic derivation index exceeds bounds
                            ChildNumber::from_normal_idx(index).map_err(|_| "Invalid index".to_string())?,
                        ],
                    )
