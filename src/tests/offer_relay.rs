...
fn close_frame_builds_close_array() {
    let frame = NostrRelayClient::close_frame("sub-9").expect("close frame");
...
fn parse_ok_frame_rejects_malformed_inputs() {
    let p = |f: &str| NostrRelayClient::parse_ok_frame(f, "id");
...
fn parse_event_frame_rejects_malformed_inputs() {
    let p = |f: &str| NostrRelayClient::parse_event_frame(f, "sub");
    assert!(p("not json").is_none(), "non-json");
