//! Nostr relay transport helpers for decentralized listing publication/discovery.

use crate::{NostrListingEvent, ZincError, LISTING_EVENT_KIND};
...
const LISTING_SCHEMA_TAG_VALUE: &str = "zinc-listing-v1";

/// Publish result for one relay.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct ListingRelayPublishResult {
...
/// Stateless helper for Nostr listing relay framing, publishing, and discovery.
pub struct NostrListingRelayClient;

impl NostrListingRelayClient {
    /// Build an `["EVENT", <event>]` frame.
    pub fn event_frame(event: &NostrListingEvent) -> Result<String, ZincError> {
        serde_json::to_string(&serde_json::json!(["EVENT", event]))
            .map_err(|e| ZincError::SerializationError(e.to_string()))
    }

    /// Build a `["REQ", sub_id, filter]` frame for listing discovery.
    pub fn req_frame(subscription_id: &str, limit: usize) -> Result<String, ZincError> {
        serde_json::to_string(&serde_json::json!([
            "REQ",
            subscription_id,
            {
                "kinds": [LISTING_EVENT_KIND],
                "#z": [LISTING_SCHEMA_TAG_VALUE],
                "limit": limit
            }
        ]))
        .map_err(|e| ZincError::SerializationError(e.to_string()))
    ...
    pub fn parse_event_frame(frame: &str, subscription_id: &str) -> Option<NostrListingEvent> {
        let value: Value = serde_json::from_str(frame).ok()?;
        let arr = value.as_array()?;
        if arr.len() != 3 {
            return None;
        ...
        let event: NostrListingEvent = serde_json::from_value(arr.get(2)?.clone()).ok()?;
        event.verify().ok()?;
        Some(event)
    }

    /// Publish one signed listing event to one relay and wait for relay `OK`.
    pub async fn publish_listing(
        relay_url: &str,
        event: &NostrListingEvent,
        timeout_ms: u64,
    ) -> Result<ListingRelayPublishResult, ZincError> {
        event.verify()?;
            ...
            while let Some(message) = socket.next().await {
                match message {
                    Ok(Message::Text(text)) => {
                        ...
                        {
                            return Ok(ListingRelayPublishResult {
    ...
    /// Publish one listing event to multiple relays concurrently.
    pub async fn publish_listing_multi(
        relay_urls: &[String],
        event: &NostrListingEvent,
        timeout_ms: u64,
    ) -> Vec<ListingRelayPublishResult> {
        let mut tasks = Vec::new();
        for relay_url in relay_urls {
            let relay = relay_url.clone();
            let event = event.clone();
            tasks.push(tokio::spawn(async move {
                match Self::publish_listing(&relay, &event, timeout_ms).await {
                    Ok(result) => result,
                    Err(err) => ListingRelayPublishResult {
                        relay_url: relay,
                        accepted: false,
    ...
    /// Discover valid listing events from a single relay.
    pub async fn discover_listing_events(
        relay_url: &str,
        options: ListingRelayQueryOptions,
    ) -> Result<Vec<NostrListingEvent>, ZincError> {
        let (mut socket, _) = connect_async(relay_url).await.map_err(|e| {
            ZincError::OfferError(format!("failed to connect relay {relay_url}: {e}"))
        })?;
        ...
        .map_err(|_| {
            ZincError::OfferError(format!(
                "relay {relay_url} timed out while discovering listings"
            ))
        })??;
    ...
    /// Discover valid listing events across multiple relays and dedupe by event id.
    pub async fn discover_listing_events_multi(
        relay_urls: &[String],
        options: ListingRelayQueryOptions,
    ) -> Vec<NostrListingEvent> {
        let mut tasks = Vec::new();
        for relay_url in relay_urls {
            let relay = relay_url.clone();
            let options = options.clone();
            tasks.push(tokio::spawn(async move {
                Self::discover_listing_events(&relay, options)
                    .await
                    .unwrap_or_default()
            }));
        }
