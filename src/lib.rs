...
impl ZincWasmWallet {
    ...
    pub fn audit_psbt(&self, psbt_base64: &str, options: JsValue) -> Result<(), JsValue> {
        ...
        let mut known_inscriptions: std::collections::HashMap<
            (bitcoin::Txid, u32),
            Vec<(String, u64)>,
        > = std::collections::HashMap::with_capacity(inner.inscriptions.len()); // PERFORMANCE OPTIMIZATION (Bolt): Pre-allocate capacity based on exact source length to prevent dynamic reallocations
