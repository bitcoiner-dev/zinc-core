...
## 2024-05-30 - [Panic Risk from unwrap on ChildNumber conversion]
**Vulnerability:** Found uses of `.unwrap()` on `bdk_wallet::bitcoin::bip32::ChildNumber::from_hardened_idx` and `ChildNumber::from_normal_idx`. This can cause the application (and particularly the WASM runtime) to panic and crash if the index exceeds `2^31 - 1`, leading to a Denial of Service (DoS).
**Learning:** Type conversions for BIP32 derivation indices from external input are prone to panic if unhandled and bounds checks are not performed.
**Prevention:** Avoid using `.unwrap()` or `.expect()` on `ChildNumber` derivation limits when handling user-provided data. Use safe fallbacks or map errors (e.g. `Result`) instead to fail securely.
